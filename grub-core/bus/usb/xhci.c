...
enum
{
  GRUB_XHCI_EHCC_CAPLEN = 0x00,	/* byte */
  GRUB_XHCI_EHCC_VERSION = 0x02,	/* word */
  GRUB_XHCI_EHCC_SPARAMS = 0x04,	/* dword */
  GRUB_XHCI_EHCC_CPARAMS = 0x08,	/* dword */
  GRUB_XHCI_EHCC_PROUTE = 0x0c,	/* 60 bits */
};

#define GRUB_XHCI_EECP_MASK     (0xff << 8)
#define GRUB_XHCI_EECP_SHIFT    8
...
enum
{
  GRUB_XHCI_SPARAMS_N_PORTS = (0xf << 0),
  GRUB_XHCI_SPARAMS_PPC = (1 << 4),	/* Power port control */
  GRUB_XHCI_SPARAMS_PRR = (1 << 7),	/* Port routing rules */
  GRUB_XHCI_SPARAMS_N_PCC = (0xf << 8),	/* No of ports per comp. */
  GRUB_XHCI_SPARAMS_NCC = (0xf << 12),	/* No of com. controllers */
  GRUB_XHCI_SPARAMS_P_IND = (1 << 16),	/* Port indicators present */
  GRUB_XHCI_SPARAMS_DEBUG_P = (0xf << 20)	/* Debug port */
};

#define GRUB_XHCI_MAX_N_PORTS     15	/* Max. number of ports */

/* Capability register CPARAMS bits */
enum
{
  GRUB_XHCI_CPARAMS_64BIT = (1 << 0),
  GRUB_XHCI_CPARAMS_PROG_FRAMELIST = (1 << 1),
  GRUB_XHCI_CPARAMS_PARK_CAP = (1 << 2)
};

#define GRUB_XHCI_N_FRAMELIST   1024
#define GRUB_XHCI_N_QH  256
#define GRUB_XHCI_N_TD  640

#define GRUB_XHCI_QH_EMPTY 1
...
enum
{
  GRUB_XHCI_COMMAND = 0x00,
  GRUB_XHCI_STATUS = 0x04,
  GRUB_XHCI_INTERRUPT = 0x08,
  GRUB_XHCI_FRAME_INDEX = 0x0c,
  GRUB_XHCI_64BIT_SEL = 0x10,
  GRUB_XHCI_FL_BASE = 0x14,
  GRUB_XHCI_CUR_AL_ADDR = 0x18,
  GRUB_XHCI_CONFIG_FLAG = 0x40,
  GRUB_XHCI_PORT_STAT_CMD = 0x44
};

/* Operational register COMMAND bits */
enum
{
  GRUB_XHCI_CMD_RUNSTOP = (1 << 0),
  GRUB_XHCI_CMD_HC_RESET = (1 << 1),
  GRUB_XHCI_CMD_FL_SIZE = (3 << 2),
  GRUB_XHCI_CMD_PS_ENABL = (1 << 4),
  GRUB_XHCI_CMD_AS_ENABL = (1 << 5),
  GRUB_XHCI_CMD_AS_ADV_D = (1 << 6),
  GRUB_XHCI_CMD_L_HC_RES = (1 << 7),
  GRUB_XHCI_CMD_AS_PARKM = (3 << 8),
  GRUB_XHCI_CMD_AS_PARKE = (1 << 11),
  GRUB_XHCI_CMD_INT_THRS = (0xff << 16)
};

/* Operational register STATUS bits */
enum
{
  GRUB_XHCI_ST_INTERRUPT = (1 << 0),
  GRUB_XHCI_ST_ERROR_INT = (1 << 1),
  GRUB_XHCI_ST_PORT_CHG = (1 << 2),
  GRUB_XHCI_ST_FL_ROLLOVR = (1 << 3),
  GRUB_XHCI_ST_HS_ERROR = (1 << 4),
  GRUB_XHCI_ST_AS_ADVANCE = (1 << 5),
  GRUB_XHCI_ST_HC_HALTED = (1 << 12),
  GRUB_XHCI_ST_RECLAM = (1 << 13),
  GRUB_XHCI_ST_PS_STATUS = (1 << 14),
  GRUB_XHCI_ST_AS_STATUS = (1 << 15)
};

/* Operational register PORT_STAT_CMD bits */
enum
{
  GRUB_XHCI_PORT_CONNECT = (1 << 0),
  GRUB_XHCI_PORT_CONNECT_CH = (1 << 1),
  ...
  GRUB_XHCI_PORT_WMASK = ~(GRUB_XHCI_PORT_CONNECT_CH
...
/* xHCI Periodic Frame Span Traversal Node */
/* Currently not supported */
...
struct grub_xhci
{
  ...
  struct grub_pci_dma_chunk *qh_chunk;	/* GRUB_XHCI_N_QH Queue Heads */
  grub_xhci_qh_t qh_virt;
  grub_uint32_t qh_phys;
  struct grub_pci_dma_chunk *td_chunk;	/* GRUB_XHCI_N_TD Transfer Descriptors */
  grub_xhci_td_t td_virt;
  grub_uint32_t td_phys;
  grub_xhci_td_t tdfree_virt;	/* Free Transfer Descriptors */
  int flag64;
...
static void
sync_all_caches (struct grub_xhci *e)
{
  if (!e)
    return;
  if (e->td_virt)
    grub_arch_sync_dma_caches (e->td_virt, sizeof (struct grub_xhci_td) *
			       GRUB_XHCI_N_TD);
  if (e->qh_virt)
    grub_arch_sync_dma_caches (e->qh_virt, sizeof (struct grub_xhci_qh) *
			       GRUB_XHCI_N_QH);
  if (e->framelist_virt)
...
grub_xhci_ehcc_read32 (struct grub_xhci *e, grub_uint32_t addr)
{
  return
...
grub_xhci_ehcc_read16 (struct grub_xhci *e, grub_uint32_t addr)
{
  return
...
grub_xhci_ehcc_read8 (struct grub_xhci *e, grub_uint32_t addr)
{
...
static inline grub_uint32_t
grub_xhci_oper_read32 (struct grub_xhci *e, grub_uint32_t addr)
{
  return
    grub_le_to_cpu32 (*
...
static inline void
grub_xhci_oper_write32 (struct grub_xhci *e, grub_uint32_t addr,
			grub_uint32_t value)
{
...
static inline grub_uint32_t
grub_xhci_port_read (struct grub_xhci *e, grub_uint32_t port)
{
  return grub_xhci_oper_read32 (e, GRUB_XHCI_PORT_STAT_CMD + port * 4);
}

static inline void
grub_xhci_port_resbits (struct grub_xhci *e, grub_uint32_t port,
			grub_uint32_t bits)
{
  grub_xhci_oper_write32 (e, GRUB_XHCI_PORT_STAT_CMD + port * 4,
			  grub_xhci_port_read (e,
					       port) & GRUB_XHCI_PORT_WMASK &
			  ~(bits));
  grub_xhci_port_read (e, port);
}

static inline void
grub_xhci_port_setbits (struct grub_xhci *e, grub_uint32_t port,
			grub_uint32_t bits)
{
  grub_xhci_oper_write32 (e, GRUB_XHCI_PORT_STAT_CMD + port * 4,
			  (grub_xhci_port_read (e, port) &
...
grub_xhci_halt (struct grub_xhci *e)
{
  grub_uint64_t maxtime;

  if ((grub_xhci_oper_read32 (e, GRUB_XHCI_STATUS) & GRUB_XHCI_ST_HC_HALTED) == 0)	/* xHCI is not halted */
    {
      /* Halt xHCI */
      grub_xhci_oper_write32 (e, GRUB_XHCI_COMMAND,
			      ~GRUB_XHCI_CMD_RUNSTOP
			      & grub_xhci_oper_read32 (e, GRUB_XHCI_COMMAND));
      /* Ensure command is written */
      grub_xhci_oper_read32 (e, GRUB_XHCI_COMMAND);
      maxtime = grub_get_time_ms () + 1000;	/* Fix: Should be 2ms ! */
      while (((grub_xhci_oper_read32 (e, GRUB_XHCI_STATUS)
	       & GRUB_XHCI_ST_HC_HALTED) == 0)
	     && (grub_get_time_ms () < maxtime));
      if ((grub_xhci_oper_read32 (e, GRUB_XHCI_STATUS)
	   & GRUB_XHCI_ST_HC_HALTED) == 0)
...
/* xHCI HC reset */
static grub_usb_err_t
grub_xhci_reset (struct grub_xhci *e)
{
  grub_uint64_t maxtime;
  ...
  grub_xhci_oper_write32 (e, GRUB_XHCI_COMMAND,
			  GRUB_XHCI_CMD_HC_RESET
			  | grub_xhci_oper_read32 (e, GRUB_XHCI_COMMAND));
  /* Ensure command is written */
  grub_xhci_oper_read32 (e, GRUB_XHCI_COMMAND);
  /* XXX: How long time could take reset of HC ? */
  maxtime = grub_get_time_ms () + 1000;
  while (((grub_xhci_oper_read32 (e, GRUB_XHCI_COMMAND)
	   & GRUB_XHCI_CMD_HC_RESET) != 0)
	 && (grub_get_time_ms () < maxtime));
  if ((grub_xhci_oper_read32 (e, GRUB_XHCI_COMMAND)
       & GRUB_XHCI_CMD_HC_RESET) != 0)
    return GRUB_USB_ERR_TIMEOUT;
		    ...
		    void *data __attribute__ ((unused)))
{
  ...
  if (pciid == GRUB_CS5536_PCIID)
      ...
      if (release != 0x30)
	{
      ...
      /* Determine xHCI EHCC registers base address.  */
  ...
  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: iobase of EHCC: %08x\n",
		(base & GRUB_XHCI_ADDR_MEM_MASK));
  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: CAPLEN: %02x\n",
		grub_xhci_ehcc_read8 (e, GRUB_XHCI_EHCC_CAPLEN));
  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: VERSION: %04x\n",
		grub_xhci_ehcc_read16 (e, GRUB_XHCI_EHCC_VERSION));
  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: SPARAMS: %08x\n",
		grub_xhci_ehcc_read32 (e, GRUB_XHCI_EHCC_SPARAMS));
  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: CPARAMS: %08x\n",
		grub_xhci_ehcc_read32 (e, GRUB_XHCI_EHCC_CPARAMS));

  /* Determine base address of xHCI operational registers */
  caplen = grub_xhci_ehcc_read8 (e, GRUB_XHCI_EHCC_CAPLEN);
  ...
  grub_dprintf ("xhci",
		...
		(base & GRUB_XHCI_ADDR_MEM_MASK) + caplen);
  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: COMMAND: %08x\n",
		grub_xhci_oper_read32 (e, GRUB_XHCI_COMMAND));
  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: STATUS: %08x\n",
		grub_xhci_oper_read32 (e, GRUB_XHCI_STATUS));
  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: INTERRUPT: %08x\n",
		grub_xhci_oper_read32 (e, GRUB_XHCI_INTERRUPT));
  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: FRAME_INDEX: %08x\n",
		grub_xhci_oper_read32 (e, GRUB_XHCI_FRAME_INDEX));
  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: FL_BASE: %08x\n",
		grub_xhci_oper_read32 (e, GRUB_XHCI_FL_BASE));
  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: CUR_AL_ADDR: %08x\n",
		grub_xhci_oper_read32 (e, GRUB_XHCI_CUR_AL_ADDR));
  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: CONFIG_FLAG: %08x\n",
		grub_xhci_oper_read32 (e, GRUB_XHCI_CONFIG_FLAG));

  /* Is there EECP ? */
  eecp_offset = (grub_xhci_ehcc_read32 (e, GRUB_XHCI_EHCC_CPARAMS)
		 & GRUB_XHCI_EECP_MASK) >> GRUB_XHCI_EECP_SHIFT;

  /* Check format of data structures requested by xHCI */
  /* XXX: In fact it is not used at any place, it is prepared for future
   * This implementation uses 32-bits pointers only */
  e->flag64 = ((grub_xhci_ehcc_read32 (e, GRUB_XHCI_EHCC_CPARAMS)
		& GRUB_XHCI_CPARAMS_64BIT) != 0);
  ...
  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: framelist mem=%p. OK\n",
		e->framelist_virt);

  /* Allocate memory for the QHs and register it in "e".  */
  e->qh_chunk = grub_memalign_dma32 (4096,
				     sizeof (struct grub_xhci_qh) *
				     GRUB_XHCI_N_QH);
  if (!e->qh_chunk)
    goto fail;
  e->qh_virt = (grub_xhci_qh_t) grub_dma_get_virt (e->qh_chunk);
  e->qh_phys = grub_dma_get_phys (e->qh_chunk);
  grub_memset ((void *) e->qh_virt, 0,
	       sizeof (struct grub_xhci_qh) * GRUB_XHCI_N_QH);

  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: QH mem=%p. OK\n",
		e->qh_virt);

  /* Allocate memory for the TDs and register it in "e".  */
  e->td_chunk = grub_memalign_dma32 (4096,
				     sizeof (struct grub_xhci_td) *
				     GRUB_XHCI_N_TD);
  if (!e->td_chunk)
    goto fail;
  e->td_virt = (grub_xhci_td_t) grub_dma_get_virt (e->td_chunk);
  e->td_phys = grub_dma_get_phys (e->td_chunk);
  grub_memset ((void *) e->td_virt, 0,
	       sizeof (struct grub_xhci_td) * GRUB_XHCI_N_TD);

  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: TD mem=%p. OK\n",
		e->td_virt);

  /* Setup all frame list pointers. Since no isochronous transfers
     are supported, they all point to the (same!) queue
     head with index 0. */
  fp = grub_cpu_to_le32 ((e->qh_phys & GRUB_XHCI_POINTER_MASK)
			 | GRUB_XHCI_HPTR_TYPE_QH);
  for (i = 0; i < GRUB_XHCI_N_FRAMELIST; i++)
    e->framelist_virt[i] = fp;
  /* Prepare chain of all TDs and set Terminate in all TDs */
  for (i = 0; i < (GRUB_XHCI_N_TD - 1); i++)
    {
      e->td_virt[i].link_td = e->td_phys + (i + 1) * sizeof (struct grub_xhci_td);
      e->td_virt[i].next_td = grub_cpu_to_le32_compile_time (GRUB_XHCI_TERMINATE);
      e->td_virt[i].alt_next_td = grub_cpu_to_le32_compile_time (GRUB_XHCI_TERMINATE);
    }
  e->td_virt[GRUB_XHCI_N_TD - 1].next_td =
    grub_cpu_to_le32_compile_time (GRUB_XHCI_TERMINATE);
  e->td_virt[GRUB_XHCI_N_TD - 1].alt_next_td =
    grub_cpu_to_le32_compile_time (GRUB_XHCI_TERMINATE);
  e->tdfree_virt = e->td_virt;
  /* Set Terminate in first QH, which is used in framelist */
  e->qh_virt[0].qh_hptr = grub_cpu_to_le32_compile_time (GRUB_XHCI_TERMINATE | GRUB_XHCI_HPTR_TYPE_QH);
  e->qh_virt[0].td_overlay.next_td = grub_cpu_to_le32_compile_time (GRUB_XHCI_TERMINATE);
  e->qh_virt[0].td_overlay.alt_next_td =
    grub_cpu_to_le32_compile_time (GRUB_XHCI_TERMINATE);
  /* Also set Halted bit in token */
  e->qh_virt[0].td_overlay.token = grub_cpu_to_le32_compile_time (GRUB_XHCI_STATUS_HALTED);
  /* Set the H bit in first QH used for AL */
  e->qh_virt[1].ep_char = grub_cpu_to_le32_compile_time (GRUB_XHCI_H);
  ...
  for (i = 1; i < GRUB_XHCI_N_QH; i++)
	...
	grub_cpu_to_le32_compile_time (GRUB_XHCI_STATUS_HALTED);
    }
  ...
  grub_dprintf ("xhci", "xHCI grub_xhci_pci_iter: QH/TD init. OK\n");
  ...
  if (grub_xhci_halt (e) != GRUB_USB_ERR_NONE)
    {
  ...
  /* Reset xHCI */
  if (grub_xhci_reset (e) != GRUB_USB_ERR_NONE)
    {
      grub_error (GRUB_ERR_TIMEOUT,
  ...
  grub_xhci_oper_write32 (e, GRUB_XHCI_FL_BASE, e->framelist_phys);
  grub_xhci_oper_write32 (e, GRUB_XHCI_CUR_AL_ADDR,
			  grub_dma_virt2phys (&e->qh_virt[1],
					       e->qh_chunk));

  /* Set ownership of root hub ports to xHCI */
  grub_xhci_oper_write32 (e, GRUB_XHCI_CONFIG_FLAG, GRUB_XHCI_CF_XHCI_OWNER);

  /* Enable both lists */
  grub_xhci_oper_write32 (e, GRUB_XHCI_COMMAND,
			  GRUB_XHCI_CMD_AS_ENABL
			  | GRUB_XHCI_CMD_PS_ENABL
			  | grub_xhci_oper_read32 (e, GRUB_XHCI_COMMAND));

  /* Now should be possible to power-up and enumerate ports etc. */
  if ((grub_xhci_ehcc_read32 (e, GRUB_XHCI_EHCC_SPARAMS)
       & GRUB_XHCI_SPARAMS_PPC) != 0)
    {				/* xHCI has port powering control */
      /* Power on all ports */
      n_ports = grub_xhci_ehcc_read32 (e, GRUB_XHCI_EHCC_SPARAMS)
	& GRUB_XHCI_SPARAMS_N_PORTS;
      for (i = 0; i < (int) n_ports; i++)
	grub_xhci_oper_write32 (e, GRUB_XHCI_PORT_STAT_CMD + i * 4,
				GRUB_XHCI_PORT_POWER
				| grub_xhci_oper_read32 (e,
							 GRUB_XHCI_PORT_STAT_CMD
							 + i * 4));
    }
...
static grub_xhci_qh_t
grub_xhci_find_qh (struct grub_xhci *e, grub_usb_transfer_t transfer)
{
  grub_uint32_t target, mask;
  int i;
  grub_xhci_qh_t qh = e->qh_virt;
  grub_xhci_qh_t head;
  grub_uint32_t qh_phys;
  grub_uint32_t qh_terminate =
    GRUB_XHCI_TERMINATE | GRUB_XHCI_HPTR_TYPE_QH;
  grub_xhci_qh_t qh_iter;

  /* Prepare part of EP Characteristic to find existing QH */
  target = ((transfer->endpoint << GRUB_XHCI_EP_NUM_OFF) |
	    transfer->devaddr) & GRUB_XHCI_TARGET_MASK;
  target = grub_cpu_to_le32 (target);
  mask = grub_cpu_to_le32_compile_time (GRUB_XHCI_TARGET_MASK);
  ...
  if (qh_phys != qh_terminate)
    qh_iter = grub_dma_phys2virt ( qh_phys & GRUB_XHCI_QHTDPTR_MASK,
      e->qh_chunk );
  else
    qh_iter = NULL;
  ...
  for (
    ...
    (qh_iter != head) && (i < GRUB_XHCI_N_QH);
    i++ )
    {
      if (target == (qh_iter->ep_char & mask))
	{		
	  /* Found proper existing (and linked) QH, do setup of QH */
	  grub_dprintf ("xhci", "find_qh: found, QH=%p\n", qh_iter);
	  grub_xhci_setup_qh (qh_iter, transfer);
	  sync_all_caches (e);
	  return qh_iter;
	}
  ...
  /* variable "i" should be never equal to GRUB_XHCI_N_QH here */
  if (i >= GRUB_XHCI_N_QH)
    { /* Something very bad happened in QH list(s) ! */
      grub_dprintf ("xhci", "find_qh: Mismatch in QH list! head=%p\n",
        head);
    }

  /* QH with target_addr does not exist, we have to find and add it */
  for (i = 2; i < GRUB_XHCI_N_QH; i++) /* We ignore zero and first QH */
    {
      if (!qh[i].ep_char)
  ...
  if (i >= GRUB_XHCI_N_QH)	/* No. */
    {
      grub_dprintf ("xhci", "find_qh: end - no free QH\n");
      return NULL;
    }
  grub_dprintf ("xhci", "find_qh: new, i=%d, QH=%p\n",
		i, &qh[i]);
  ...
  grub_xhci_setup_qh (&qh[i], transfer);
  ...
  head->qh_hptr = grub_cpu_to_le32 (GRUB_XHCI_HPTR_TYPE_QH
...
grub_xhci_alloc_td (struct grub_xhci *e)
{
  grub_xhci_td_t ret;
  ...
  if (!e->tdfree_virt)
    {
      grub_dprintf ("xhci", "alloc_td: end - no free TD\n");
      return NULL;
    }
...
static void
grub_xhci_free_td (struct grub_xhci *e, grub_xhci_td_t td)
{
...
static void
grub_xhci_free_tds (struct grub_xhci *e, grub_xhci_td_t td,
		    grub_usb_transfer_t transfer, grub_size_t * actual)
{
  ...
  *actual = 0;
  ...
  for (i = 0; td; i++)
    {
      grub_xhci_td_t tdprev;

      token = grub_le_to_cpu32 (td->token);
      to_transfer = (token & GRUB_XHCI_TOTAL_MASK) >> GRUB_XHCI_TOTAL_OFF;
   ...
   * xHCI can probably split transactions into smaller parts then
...
static grub_xhci_td_t
grub_xhci_transaction (struct grub_xhci *e,
			  ...
			  grub_usb_transfer_t transfer)
{
  ...
  status = grub_xhci_oper_read32 (e, GRUB_XHCI_STATUS);
  ...
  cdata->qh_virt = grub_xhci_find_qh (e, transfer);
  if (!cdata->qh_virt)
    {
   ...
   * allocate it first. */
  cdata->td_alt_virt = grub_xhci_alloc_td (e);
  ...
  for (i = 0; i < transfer->transcnt; i++)
    {
      ...
      td = grub_xhci_transaction (e, tr->pid, tr->toggle, tr->size,
      ...
      if (!td)			/* de-allocate and free all */
	{
	  ...
	  if (cdata->td_first_virt)
	    grub_xhci_free_tds (e, cdata->td_first_virt, NULL, &actual);
			...
			grub_usb_transfer_t transfer, grub_size_t * actual)
{
  ...
  grub_xhci_free_tds (e, cdata->td_first_virt, transfer, actual);
  grub_xhci_free_td (e, cdata->td_alt_virt);
  grub_free (cdata);
  ...
  grub_xhci_oper_write32 (e, GRUB_XHCI_COMMAND,
			  GRUB_XHCI_CMD_RUNSTOP | GRUB_XHCI_CMD_AS_ENABL
			  | GRUB_XHCI_CMD_PS_ENABL
			  | grub_xhci_oper_read32 (e, GRUB_XHCI_COMMAND));
  /* Ensure command is written */
  grub_xhci_oper_read32 (e, GRUB_XHCI_COMMAND);
			   ...
			   grub_usb_transfer_t transfer)
{
  ...
  if (((grub_xhci_oper_read32 (e, GRUB_XHCI_STATUS) &
      GRUB_XHCI_ST_HC_HALTED) != 0) ||
    (!interrupt && ((grub_xhci_oper_read32 (e, GRUB_XHCI_STATUS) &
      (GRUB_XHCI_ST_AS_STATUS | GRUB_XHCI_ST_PS_STATUS)) == 0)))
    {
      grub_xhci_pre_finish_transfer (transfer);
      grub_xhci_free_tds (e, cdata->td_first_virt, transfer, &actual);
  ...
  if (cdata->qh_virt->ep_cap & GRUB_XHCI_SMASK_MASK)
  ...
  else
    {
      ...
      grub_xhci_oper_write32 (e, GRUB_XHCI_COMMAND,
                              ~GRUB_XHCI_CMD_AS_ADV_D
                              & grub_xhci_oper_read32 (e, GRUB_XHCI_COMMAND));
      grub_xhci_oper_write32 (e, GRUB_XHCI_STATUS,
...
grub_xhci_hubports (grub_usb_controller_t dev)
{
  ...
  portinfo = grub_xhci_ehcc_read32 (e, GRUB_XHCI_EHCC_SPARAMS)
    & GRUB_XHCI_SPARAMS_N_PORTS;
		      ...
		      unsigned int port, unsigned int enable)
{
  ...
  grub_dprintf ("xhci",
		...
		e->iobase, port, grub_xhci_port_read (e, port));
  ...
  grub_xhci_port_resbits (e, port, GRUB_XHCI_PORT_ENABLED);
  endtime = grub_get_time_ms () + 1000;
  while (grub_xhci_port_read (e, port) & GRUB_XHCI_PORT_ENABLED)
  ...
  grub_xhci_port_setbits (e, port, GRUB_XHCI_PORT_RESET);
  grub_millisleep (50);

  /* Reset RESET bit and wait for the end of reset */
  grub_xhci_port_resbits (e, port, GRUB_XHCI_PORT_RESET);
  endtime = grub_get_time_ms () + 1000;
  while (grub_xhci_port_read (e, port) & GRUB_XHCI_PORT_RESET)
    if (grub_get_time_ms () > endtime)
      return GRUB_USB_ERR_TIMEOUT;
  ...
  if ((grub_xhci_port_read (e, port) & GRUB_XHCI_PORT_ENABLED) != 0)	/* yes! */
    {
  ...
  else				/* no... */
    {
      ...
      grub_xhci_port_setbits (e, port, GRUB_XHCI_PORT_OWNER);
...
grub_xhci_detect_dev (grub_usb_controller_t dev, int port, int *changed)
{
  ...
  status = grub_xhci_port_read (e, port);

  /* Connect Status Change bit - it detects change of connection */
  if (status & GRUB_XHCI_PORT_CONNECT_CH)
    {
      *changed = 1;
      /* Reset bit Connect Status Change */
      grub_xhci_port_setbits (e, port, GRUB_XHCI_PORT_CONNECT_CH);
    }
  else
    *changed = 0;

  if (!(status & GRUB_XHCI_PORT_CONNECT))
...
grub_xhci_restore_hw (void)
{
  ...
  for (e = xhci; e; e = e->next)
	...
	grub_error (GRUB_ERR_TIMEOUT, "restore_hw: xHCI reset timeout");
      ...
      /* Set ownership of root hub ports to xHCI */
      grub_xhci_oper_write32 (e, GRUB_XHCI_CONFIG_FLAG,
			      GRUB_XHCI_CF_XHCI_OWNER);
